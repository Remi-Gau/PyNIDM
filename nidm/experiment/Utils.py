...
def find_in_namespaces(search_uri, namespaces):
    ...
    return next((uris for uris in namespaces if uris.uri == search_uri), False)
        ...
        if predicate != URIRef(Constants.PROV['qualifiedAssociation']):
            ...
            if (validators.url(objects)) and (predicate != Constants.PROV['Location']):
                ...
                try:
                    ...
                    elif found_uri := find_in_namespaces(
        ...
        for r_obj in r.objects(predicate=Constants.PROV['hadRole']):
            ...
            if r_obj.identifier == URIRef(Constants.NIDM_PARTICIPANT.uri):
                ...
                for agent_obj in r.objects(predicate=Constants.PROV['agent']):
                    ...
                    if found_uri := find_in_namespaces(
                        search_uri=URIRef(obj_nm), namespaces=namespaces
                    ):
...
def AddPDEToInterlex(ilx_obj,label,definition,units, min, max, datatype, isabout=None, categorymappings=None):
    ...
    elif isabout is not None:
        return ilx_obj.add_pde(
            label=label,
            definition=definition,
            predicates={
                uri_datatype: datatype,
...
def load_nidm_terms_concepts():
    ...
    try:
        ...
        logging.info(f"Error opening {concept_url} used concepts file..continuing")
        return None
...
def load_nidm_owl_files():
    ...
    for resource in owls:
        ...
        try:
            ...
            logging.info(f"Error opening {resource} owl file..continuing")
            continue
...
def fuzzy_match_concepts_from_nidmterms_jsonld(json_struct,query_string):
    ...
    for entry in json_struct['terms']:
        match_scores[entry['label']] = {
            'score': fuzz.token_sort_ratio(query_string, entry['label'])
        }
...
def fuzzy_match_terms_from_cogatlas_json(json_struct,query_string):
    ...
    for entry in json_struct:
        ...
        match_scores[entry['name']] = {
            'score': fuzz.token_sort_ratio(query_string, entry['name'])
        }
...
def redcap_datadictionary_to_json(redcap_dd_file,assessment_name):
    ...
    for index,row in redcap_dd.iterrows():
        ...
        if pd.isnull(row['Choices OR Calculations']):
            ...
            json_map[current_tuple]['levels'] = [str(row['Choices OR Calculations'])]
        else:
...
def detect_json_format(json_map):
    ...
    for key,value in json_map.keys():
        if "DD(" in key:
            return "REPROSCHEMA" if "responseOptions" in value.keys() else "OLD_PYNIDM"
        else:
            return "BIDS"
...
def match_participant_id_field(source_variable):
    ...
    return (
        "participant_id" in source_variable.lower()
        or "subject_id" in source_variable.lower()
        or (
            ("participant" in source_variable.lower())
            and ("id" in source_variable.lower())
        )
        or (
            ("subject" in source_variable.lower())
            and ("id" in source_variable.lower())
        )
        or (
            ("sub" in source_variable.lower())
            and ("id" in source_variable.lower())
        )
    )
